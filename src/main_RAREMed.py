...
def get_args():
    ...
    parser.add_argument(
        ...
        "--pretrain_nsp",
        action="store_true",
        help="whether to use nsp pretrain",
    )
    parser.add_argument(
        "-mask",
        "--pretrain_mask",
        action="store_true",
        help="whether to use mask prediction pretrain",
    )
    parser.add_argument(
        "--pretrain_epochs", type=int, default=20, help="number of pretrain epochs"
    )
    ...
    )  # 增大layers，训练速度不变，性能略提高，继续增大反而会降低性能
    parser.add_argument(
        "--nhead", type=int, default=4, help="number of encoder head"
    )  # 实验有问题，增加head，训练速度不变，性能略提高
    parser.add_argument(
        "--batch_size", type=int, default=1, help="batch size during training"
    )  # 增大batch较大影响性能，可看作正则化的一种，batch小，有过拟合风险。
    parser.add_argument(
        "--adapter_dim", type=int, default=128, help="dimension of adapter layer"
    )  #

    parser.add_argument(
        "--lr", type=float, default=1e-5, help="learning rate"
    )  # 学习率增大容易过拟合，过大则不收敛（loss不怎么下降）
    parser.add_argument(
        "--dropout",
        type=float,
        default=0.3,
        help="dropout probability of transformer encoder",
    )  # 严重影响过拟合程度。drop小，训练集loss下降快，但过拟合严重。但drop太大会导致拟合不足，性能下降。
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.005,
        help="weight of multilabel_margin_loss",
    )  # 严重影响性能。增大weight_multi会提高药物数量（提高正样本预测值），进而提高ddi rate。增大multi会增大bce loss。性能先上升后下降，0.01以后明显影响性能
    parser.add_argument(
        "--weight_ddi", type=float, default=0.1, help="weight of ddi loss"
    )  # weight_ddi 越大，loss越高，推荐的药物越少。性能先上升后下降。0.5以上开始明显影响jaccard
    ...
    parser.add_argument(
        ...
        "--patient_seperate",
        action="store_true",
        help="whether to combine diseases and procedures",
    )
    parser.add_argument(
        "-e",
        "--seg_rel_emb",
...
def evaluator(
    ...
    if args.test:
        ...
        for i in range(5):
            logging.info(
                str(i + 1)
                + f"visit\t mean: {np.mean(ja_visit[i]):.4}, std: {np.std(ja_visit[i]):.4}"
            )

    # ddi rate
    ddi_rate = ddi_rate_score(
        smm_record, path=ddi_adj_path
    )  # if (epoch != 0) | (mode=='test')  else 0.0
    ...
    loss_val_sum = (
        (1 - args.weight_multi) * loss_val_bce
...
def evaluator_mask(model, data_val, voc_size, epoch, device, mode="pretrain"):
    model.eval()
    loss_val = 0
    dis_ja_list, dis_prauc_list, dis_p_list, dis_r_list, dis_f1_list = [
        [] for _ in range(5)
    ]
    pro_ja_list, pro_prauc_list, pro_p_list, pro_r_list, pro_f1_list = [
        [] for _ in range(5)
    ]
    ...
    for batch in tqdm(data_val, ncols=60, desc=mode, total=len_val):
        ...
        avg_dis_ja, avg_dis_prauc, avg_dis_p, avg_dis_r, avg_dis_f1 = (
            np.mean(dis_ja_list),
            np.mean(dis_prauc_list),
            np.mean(dis_p_list),
            np.mean(dis_r_list),
            np.mean(dis_f1_list),
        )
        avg_pro_ja, avg_pro_prauc, avg_pro_p, avg_pro_r, avg_pro_f1 = (
            np.mean(pro_ja_list),
            np.mean(pro_prauc_list),
            np.mean(pro_p_list),
            np.mean(pro_r_list),
            np.mean(pro_f1_list),
        )
        avg_ja, avg_prauc, avg_p, avg_r, avg_f1 = (
            (avg_dis_ja + avg_pro_ja) / 2,
            (avg_dis_prauc + avg_pro_prauc) / 2,
            (avg_dis_p + avg_pro_p) / 2,
            (avg_dis_r + avg_pro_r) / 2,
            (avg_dis_f1 + avg_pro_f1) / 2,
        )
...
def evaluator_nsp(model, data_val, data, epoch, device, mode="pretrain_nsp"):
    ...
    for batch_data in tqdm(data_val, ncols=60, desc=mode, total=len(data_val)):
        ...
        loss = F.cross_entropy(
            result, torch.tensor(nsp_target, device=device, dtype=torch.float32)
        )
...
def main(args):
    ...
    save_dir = os.path.join(
        log_directory_path, "log" + str(log_save_id) + "_" + args.note
    )
    logging_config(
        folder=save_dir,
    ...
    if (
        not (args.pretrain_mask or args.pretrain_nsp)
    ...
    for epoch in range(EPOCH):
        ...
        logging.info(
            f"""loss_train_all:{loss_train_all:.4f}, loss_train_bce: {loss_train_bce:.4f}, loss_train_multi:{loss_train_multi:.4f}, loss_train_ddi:{loss_train_ddi:.4f}
    ...
    torch.save(
        ...
        open(
            os.path.join(
                save_dir,
                "Epoch_{}_JA_{:.4}_DDI_{:.4}.model".format(
                    best_epoch, best_ja, best_ddi_rate
                ),
            ),
            "wb",
        ),
...
def main_mask(
    ...
    for epoch in range(EPOCH):
        ...
        loss_train = 0
        for batch in tqdm(
            data_train, ncols=60, desc="pretrain_mask", total=len(data_train)
        ):
            batch_size = len(batch)
            if args.mask_prob > 0:
            ...
            loss_multi = F.multilabel_margin_loss(
                result, torch.tensor(multi_target, device=device)
            )
        ...
        logging.info(
            f"Training Loss_mask: {loss_train:.4f}, Validation Loss_mask: {loss_val:.4f}, best_ja: {best_ja_mask:.4f} at epoch {best_epoch_mask}\n"
        )
...
def main_nsp(
    ...
    for epoch in range(EPOCH):
        ...
        loss_train /= len(data_train)
        # validation
        precision, loss_val = evaluator_nsp(
            model, data_val, data, epoch, device, mode="pretrain_nsp"
        )
        loss_val /= len(data_val)
        if precision > best_prc_nsp:
            best_epoch_nsp, best_prc_nsp = epoch, precision
        logging.info(
            f"Epoch {epoch:03d}   prc: {precision:.4}, best_prc: {best_prc_nsp:.4f} at epoch {best_epoch_nsp}, Training Loss_nsp: {loss_train:.4f}, Validation Loss_nsp: {loss_val:.4f}\n"
        )
